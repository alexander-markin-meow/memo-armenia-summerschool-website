...
test('parses only supported collection, research, and project hash routes', () => {
  ...
  assert.deepEqual(parseHashRoute('#/en/research'), { kind: 'research', locale: 'en' });
...
test('preserves project context while changing language', () => {
  ...
  assert.equal(localizedHashRoute({ kind: 'research', locale: 'en' }, 'hy'), '#/hy/research');
});
